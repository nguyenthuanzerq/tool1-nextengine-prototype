...
**Step 5: Start Ngrok (For Public URL & HTTPS)**
Since OAuth providers (like NextEngine and Yahoo) and Webhooks require a public HTTPS URL, you must run Ngrok to expose your local server:
```bash
ngrok http 8000
```
*Keep this terminal window open and note down the forwarding URL provided by Ngrok (e.g., `https://abcdef123.ngrok-free.app`).*

**Step 6: Update Environment Configuration (.env)**
Open the generated `.env` file. You need to update your `APP_URL` and `NEXT_ENGINE_REDIRECT_URI` to use your new Ngrok HTTPS URL. This is crucial because `AppServiceProvider` forces HTTPS when running through Ngrok.
```env
APP_URL=https://abcdef123.ngrok-free.app

NEXT_ENGINE_BASE_URI=https://base.next-engine.org
NEXT_ENGINE_API_URI=https://api.next-engine.org
NEXT_ENGINE_REDIRECT_URI=${APP_URL}/nextengine/callback
```
...
Open a **new** terminal window (leave Ngrok running in the first one) and start Laravel:
```bash
php artisan serve
```
...
### Default Admin Login
*   **Dashboard URL:** `<your-ngrok-url>/btoc` (requires login)
